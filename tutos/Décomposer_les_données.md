dépendance fonctionnelles
numero_commande --> date_commande, nom_client, email_client , id_client
id_client --> email_client, nom_client
id_produit --> nom_produit , prix_produit
id_produit + numero_commande --> quantite_commmande
Table Client :
Client ( id_client email_client, nom_client )
Table Commande:
Commande ( numero_commande, date_commande, nom_client, email_client, id_client )
Table Produit :
Produit( id_produit, nom_produit, prix_produit )
Table Commande_Produit :
Commande_Produit ( id_produit, numero_commande, quantite_commmande )